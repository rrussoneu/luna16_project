...
raw_cache = getCache('final_cache_classification')
...
def check_validity(tup):
    mhd_path = list(pathlib.Path('luna_data').glob(f'subset*/{tup.series_uid}.mhd'))
    return (tup, len(mhd_path) > 0)
...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    with Pool() as pool:
      results = pool.map(check_validity, candidateInfo_list)

    valid_list = [tup for tup, is_valid in results if is_valid]
    num_invalid = len(candidateInfo_list) - len(valid_list)
